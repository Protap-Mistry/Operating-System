...
int main()
{
    int n, process[MAX], arrival_t[MAX], burst_t[MAX], keep_burst_t[MAX], completion_t[MAX], turned_around_t[MAX], waiting_t[MAX];
    ...
    cout<<endl<<"Process(es)"<<"\t"<<"Arrival_time"<<"\t"<<"Burst_time"<<"\t"<<"Completion_time"<<"\t"<<"Turned_around_time"<<"\t"<<"Waiting_time"<<endl<<endl;

    for(int i=0; i<n; i++)
    {
        cout<<"Process["<<process[i]<<"]: "<<"\t\t"<<arrival_t[i]<<"\t\t"<<keep_burst_t[i]<<"\t\t"<<completion_t[i]<<"\t\t"<<turned_around_t[i]<<"\t\t"<<waiting_t[i]<<endl;
