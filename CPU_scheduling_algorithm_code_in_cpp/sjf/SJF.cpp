...
int main()
{
    ...
    ///Selection sort for sorting burst time with process
     for(int i=0; i<n; i++)
    {
        value= i;
        for(int j=i+1; j<n; j++)
        {
            if(burst_time[j]<burst_time[value])
            {
                value=j;
            }
        }
        ...
        temp=burst_time[i];
        burst_time[i]=burst_time[value];
        burst_time[value]=temp;

        temp=process[i];
        process[i]=process[value];
        process[value]=temp;
    }
